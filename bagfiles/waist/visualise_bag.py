...
# Plot joint position on the first subplot
ax1.step(time[2], data[2], '-', label='ReAIC Joint Position')
ax1.step(time[1], data[1], '-', label='AIC Joint Position')
ax1.step(REF_timestamps, REF_datavalues, 'k-', label='Desired Position')
ax1.hlines(0.0, 0.0, xmax=30.0, color='k', linestyle='--')
ax1.set_ylabel('Joint Position (rad)')
ax1.legend()
...
# Plot control signal on the second subplot
ax2.step(control_time[2], control_signal[2], '-', label='ReAIC Control Signal')
ax2.step(control_time[1], control_signal[1], '-', label='AIC Control Signal')
ax2.set_xlabel('Time (s)')
ax2.set_ylabel('Control Signal (v)')
ax2.legend()
ax2.grid(True)  # Turn on the grid for the first subplot
fig.savefig('/home/alon/Documents/thesis/Tuning Parameter Influences/ReAIC_AIC_Step_response.pdf')
